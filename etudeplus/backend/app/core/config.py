...
def get_secret(secret_name: str, default: str = "") -> str:
    ...
    # 2. Check Vault if enabled
    if os.getenv("VAULT_ENABLED", "false").lower() == "true":
        try:
            from app.core.vault import get_secret_with_fallback
            return get_secret_with_fallback(
                vault_path=f"schoolflow/{secret_name.lower()}",
                env_var=secret_name,
                default=default
            )
        except Exception as e:
            logger.warning(f"Vault lookup failed for {secret_name}: {e}")
